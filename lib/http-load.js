...
exports.HttpLoad = function(filename, requestsToPerform, concurrent, debug, recordHeaders, cacheDir) {
...
exports.HttpLoad.prototype = {
  ...
  start: function() {
	...
	that.urls = fs.readFileSync(that.filename, 'utf8').split('\n');
	that.urls.pop();

	if (that.cacheDir) {
	  debug('Using cache, directory: ' + that.cacheDir);
	  var count = that.urls.length;
	  that.urls.forEach(function(url) {
	    var filepart = url.match('http://.*/(.*)')[1];
	    var filename = path.join(that.cacheDir, filepart);
	    hashFile(filename, function(hash) {
	      that.cachedHashes[url] = hash;
	      if (--count === 0) {
  ...
  randomizeRequests: function() {
    ...
    for (i = 0; i < this.requestsToPerform; i++) {
      request = {};
      var randomIndex = Math.floor(Math.random() * this.urls.length);
      request.url = this.urls[randomIndex];
  ...
  buildResults: function() {
	...
	that.maxRequestTime = request.requestTime;
      ...
      if (that.cacheDir) {
	if (request.response.hash === that.cachedHashes[request.url]) {
	  that.successes++;
	} else {
	  that.failures++;
